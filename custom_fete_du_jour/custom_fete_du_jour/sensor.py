...
class FeteDuJourSensor(Entity):
    ...
    def update(self):
        ...
        for s in saint_jour.keys():
            if self._fete == 'today_fete_du_jour':
                if s == now:
                    reponse = saint_jour[now]
            if self._fete == 'demain_fete_du_jour':
                if s == date_plus_un:
                    reponse = saint_jour[date_plus_un]

        for s in anniversaire.keys():
            if self._fete == 'personne_anniversaire':
                if s == now:
                    reponse = anniversaire[now]
            if self._fete == 'demain_personne_anniversaire':
                if s == date_plus_un:
                    reponse = anniversaire[date_plus_un]

        self._state = reponse
        _LOGGER.debug(" update " + str(self._fete) + str(" : ") + str(reponse))